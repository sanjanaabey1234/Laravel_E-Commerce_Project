...
<div class="container">
   ...
   <section class="add-product">
      <form action="{{ route('seller.store.Fruits') }}" method="post" class="add-product-form" enctype="multipart/form-data">
         @csrf
   ...
   <section class="display-product-table mt-5">
      ...
      <div class="table-responsive">
         ...
         <tbody id="product-table-body">
            ...
            <tr data-id="{{ $fruit->fruit_id }}">
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="product-image"><img src="{{ asset($fruit->image_path) }}" class="rounded img-thumbnail" style="height: 80px; width: 80px; object-fit: cover;"></td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="product-name">{{ $fruit->fruit_name }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="product-description">{{ $fruit->description }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="stock-quantity">{{ $fruit->stock_quantity }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="product-price">{{ $fruit->price }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="total-amount">{{ $fruit->Amount }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;" id="created_at">{{ $fruit->created_at }}</td>
               <td style="padding: 10px; text-align: center; background-color: #e9ecef; border: 1px solid #dee2e6;">
                  ...
                  <form id="delete-form-{{ $fruit->fruit_id }}" action="{{ route('seller.delete.Fruit', $fruit->fruit_id) }}" method="POST" style="display:inline;">
                     @csrf
                     @method('DELETE')
                     <button type="button" class="delete-btn" onclick="confirmDelete({{ $fruit->fruit_id }})">Delete</button>
                 </form>
   ...
   <section class="edit-form-container" id="edit-form-container">
      ...
      <form action="{{ route('seller.update.Fruit') }}" method="post" enctype="multipart/form-data">
         @csrf
         <h5>Edit Fruit</h3>
   ...
   <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        unitPriceInput.addEventListener('input', calculateTotalPrice);
         ...
         const fruitIdUpdate = document.querySelector('input[name="fruit_id"]');
